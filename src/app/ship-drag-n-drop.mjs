...
function dragNDropShips() {
  const shipContainers = document.querySelectorAll(
    "div.ship-menu div#ship-container",
  );
  const shipSquares = document.querySelectorAll(
    "div#ship-container span#ship-square",
  );
  ...
  const board = document.querySelector(
    "main div.player-lobby div#player-board",
  );
...
function buildNodeArray(e) {
  ...
  let nodeArray = [
    document.querySelector(
      `main div.player-lobby div#player-board span[value='${[x, y]}']`,
    ),
  ];
  ...
  const makeHorizontalArray = () => {
    if (x >= grabbedBlock) {
      for (let i = 1; i <= grabbedBlock; i++) {
        ...
        nodeArray.unshift(
          document.querySelector(
            `main div.player-lobby div#player-board span[value='${cords}']`,
          ),
        );
      }
...
function isPreOccupied(e) {
  ...
  for (let i = 0; i < e.length && !isOccupied; i++) {
    ...
    if (
      ships.some((ship) =>
        ship.position.some((cords) => cords[0] === x && cords[1] === y),
      )
    ) {
